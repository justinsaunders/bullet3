/*
Convex hull computer for the Bullet Physics Library, http://bulletphysics.org
...
#include "bullet_physics/util/bt_aligned_allocator.h"
	...
	public:
		...
		class Rational64
		{
			...
			public:
				...
				btScalar toScalar() const
				{
					return sign * ((denominator == 0) ? VM_FLT_MAX : (btScalar) numerator / denominator);
				}
		};
		...
		class Rational128
		{
			...
			public:
				...
				btScalar toScalar() const
				{
					return sign * ((denominator.getSign() == 0) ? VM_FLT_MAX : numerator.toScalar() / denominator.toScalar());
				}
		};
		...
		class Edge
		{
			public:
				...
				void link(Edge* n)
				{
					VMASSERT(reverse->target == n->reverse->target);
					next = n;
					n->prev = this;
				}
...
bool btConvexHullInternal::mergeProjection(IntermediateHull& h0, IntermediateHull& h1, Vertex*& c0, Vertex*& c1)
{
	...
	{
		VMASSERT(v0->point.z < v1->point.z);
		Vertex* v1p = v1->prev;
		if (v1p == v1)
		{
			c0 = v0;
			if (v1->edges)
			{
				VMASSERT(v1->edges->next == v1->edges);
				v1 = v1->edges->target;
				VMASSERT(v1->edges->next == v1->edges);
			}
			c1 = v1;
...
{
	VMASSERT(prev->reverse->target == next->reverse->target);
	if (prev->next == next)
	{
		if (prev->prev == next)
		{
	...
	if (e)
	{
		do
		{
			if (e->copy > mergeStamp)
			{
				...
				if (cot.isNaN())
				{
					VMASSERT(ccw ? (t.dot(s) < 0) : (t.dot(s) > 0));
				}
				else
				{
					int cmp;
...
void btConvexHullInternal::findEdgeForCoplanarFaces(Vertex* c0, Vertex* c1, Edge*& e0, Edge*& e1, Vertex* stop0, Vertex* stop1)
{
	...
	VMASSERT(!start1 || (start1->target->point.dot(normal) == dist));
	Point64 perp = s.cross(normal);
	VMASSERT(!perp.isZero());
	...
	if (e0)
	{
		while (e0->target != stop0)
		{
			...
			VMASSERT(e->target->point.dot(normal) == dist);
			if (e->copy == mergeStamp)
			{
				break;
			}
		...
		while (true)
		{
			...
			if (e1 && (e1->target != stop1))
			{
				...
				if (f1->copy > mergeStamp)
				{
					...
					{
						VMASSERT((e1 == start1) && (d1.dot(normal) < 0));
					}
				}
			}
		...
		while (true)
		{
			...
			if (e0 && (e0->target != stop0))
			{
				...
				if (f0->copy > mergeStamp)
				{
					...
					{
						VMASSERT((e0 == start0) && (d0.dot(normal) < 0));
					}
				}
			}
...
btVector3 btConvexHullInternal::toBtVector(const Point32& v)
{
	...
	return VMMUL(p,scaling);
}

btVector3 btConvexHullInternal::getBtNormal(Face* face)
{
	return VMNORMALIZED(VMCROSS(toBtVector(face->dir0),toBtVector(face->dir1)));
}
...
btScalar btConvexHullInternal::shrink(btScalar amount, btScalar clampAmount)
{
	...
	hullCenter = VMMUL(hullCenter,scaling);

	int faceCount = faces.size();

	if (clampAmount > 0)
	{
		btScalar minDist = VM_FLT_MAX;
		for (int i = 0; i < faceCount; i++)
		{
			btVector3 normal = getBtNormal(faces[i]);
			btScalar dist = VMDOT(normal,(toBtVector(faces[i]->origin) - hullCenter	));
			if (dist < minDist)
			{
				minDist = dist;
			}
...
bool btConvexHullInternal::shiftFace(Face* face, btScalar amount, btAlignedObjectArray<Vertex*> stack)
{
	...
	if (cmp >= 0)
	{
		...
		do
		{
			...
			VMASSERT(dot.compare(origDot) <= 0);
#ifdef DEBUG_CONVEX_HULL
...
#endif
			e = e->reverse->prev;
			VMASSERT(e != intersection->reverse);
